...
[![Deploy](https://img.shields.io/badge/deploy-Vercel-black?logo=vercel)](https://ecoflow-alpha.vercel.app)
...
### AI Coach and Weather Features (Optional)

To enable real AI-powered coaching, weather-based suggestions, and air quality data:
...
Dependencies are only needed for dev tooling (linting). The app itself is vanilla HTML, CSS, and JavaScript.

---

## API

Third-party API calls to Gemini and OpenWeatherMap are proxied through Vercel serverless functions. API keys are injected server-side and never exposed to the browser, with automatic fallback to `localStorage`-based keys if server keys aren't configured.
...
{ "message": "How can I reduce food waste?", "systemPrompt": "..." }
```

Proxies Gemini API calls server-side. Returns the AI-generated response text. Falls back to `localStorage` API key or simulated responses if the server key isn't configured.
...
Proxies OpenWeatherMap API calls server-side. Returns structured weather data (temperature, conditions, humidity, wind). Automatically falls back to `localStorage` key if the server key isn't configured.
...
```json
{ "gemini": true, "openweathermap": false }
```

Used by the Settings page to display status badges.
...
|----------|----------|------------|
| `GEMINI_API_KEY` | No | Google Gemini key from [aistudio.google.com/apikey](https://aistudio.google.com/apikey) |
| `OPENWEATHER_API_KEY` | No | OpenWeatherMap key from [openweathermap.org/api](https://openweathermap.org/api) |
